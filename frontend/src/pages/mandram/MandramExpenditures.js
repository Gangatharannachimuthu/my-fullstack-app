...
import { expenditureService } from '../../services/mandramApi';
import IconButton from './IconButton';

function MandramExpenditures({ meetingId }) {
  ...
  const refresh = useCallback(async () => {
    const [catRes, entriesRes] = await Promise.all([
      expenditureService.getCategories(),
      meetingId ? expenditureService.getForMeeting(meetingId) : Promise.resolve({ data: [] }),
    ]);
  ...
  const handleAdd = async (e) => {
    ...
    try {
      setError('');
      await expenditureService.create(meetingId, { category_id: Number(categoryId), amount: Number(amount), note });
      setAmount('');
      setNote('');
      await refresh();
    } catch (err) {
  ...
  const saveEdit = async (entryId) => {
    try {
      setError('');
      await expenditureService.update(entryId, {
      ...
      setError(err.response?.data?.error || 'Failed to update expenditure');
    }
  };

  const handleDelete = async (entryId) => {
    if (!window.confirm('Remove this expenditure entry?')) return;
    try {
      setError('');
      await expenditureService.remove(entryId);
      await refresh();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to remove expenditure');
    }
  };
  ...
  return (
    <div className="card">
      ...
      {entries.length === 0 ? (
        <p>No expenditure recorded for this meeting.</p>
      ) : (
...
export default MandramExpenditures;
